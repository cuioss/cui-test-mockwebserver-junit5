---
name: release
description: Cut a cui-test-mockwebserver-junit5 release — bump .github/project.yml version, open and merge the release PR, wait for the automated Release workflow, verify the release landed, then reformat the generated GitHub release notes
user-invocable: true
...
Cuts a new cui-test-mockwebserver-junit5 release end-to-end: determine the version, open the version-bump PR
...
```yaml
consumers:
  - cuioss-parent-pom:version.cui.test.mockwebserver
  - nifi-extensions:version.cui.test.mockwebserver
```
...
```bash
gh pr list --repo cuioss/cui-test-mockwebserver-junit5 --state open --json number,title,isDraft
```
- **No open PRs** → proceed.
...
if ! gh label list --repo cuioss/cui-test-mockwebserver-junit5 --search skip-bot-review --json name --jq '.[].name' \
     | grep -qx skip-bot-review; then
  gh label create skip-bot-review --repo cuioss/cui-test-mockwebserver-junit5 \
    --description "Skip automated bot review" --color ededed
fi
gh pr create --repo cuioss/cui-test-mockwebserver-junit5 --base main \
...
```bash
gh pr checks <pr#> --repo cuioss/cui-test-mockwebserver-junit5 --watch
```
...
- Every review comment MUST get a reply and MUST be resolved — fix it and say so, or explain
  ...
  ```bash
  gh api --paginate repos/cuioss/cui-test-mockwebserver-junit5/pulls/<pr#>/comments
  ```
- **Unresolved review threads block the merge.** A PR can show every check green and still
  ...
  ```bash
  gh api graphql --paginate -f query='query($endCursor:String){repository(owner:"cuioss",name:"cui-test-mockwebserver-junit5"){pullRequest(number:<pr#>){reviewThreads(first:100,after:$endCursor){pageInfo{hasNextPage endCursor} nodes{id isResolved}}}}}'
...
```bash
gh run list --repo cuioss/cui-test-mockwebserver-junit5 --workflow "Release" --limit 3 \
  --json status,conclusion,displayTitle,databaseId
gh run watch <databaseId> --repo cuioss/cui-test-mockwebserver-junit5
```

### Step 11 — Verify the release landed

```bash
gh release view <version> --repo cuioss/cui-test-mockwebserver-junit5 --json tagName,name,isDraft,createdAt
git fetch --tags && git tag --list <version>
curl -sfI https://repo1.maven.org/maven2/de/cuioss/test/cui-test-mockwebserver-junit5/<version>/cui-test-mockwebserver-junit5-<version>.pom \
...
```bash
mkdir -p .plan/temp
gh release view <version> --repo cuioss/cui-test-mockwebserver-junit5 --json body --jq .body \
  > .plan/temp/release-<version>-orig.md
# ...build the reformatted body in .plan/temp/release-<version>.md...
gh release edit <version> --repo cuioss/cui-test-mockwebserver-junit5 --notes-file .plan/temp/release-<version>.md
```
