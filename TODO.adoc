...
* [ ] Add ParameterResolver interface to MockWebServerExtension
* [ ] Implement `supportsParameter` method to check parameter types:
...
* [ ] Add caching mechanism to avoid creating multiple instances
* [ ] Add proper exception handling for unsupported parameter types
* [ ] Ensure thread safety for concurrent test execution
* [ ] Maintain backward compatibility with existing MockWebServerHolder approach
...
* [ ] Modify `beforeEach` method to check for HTTPS configuration
* [ ] Implement validation to ensure at least one key material provider is specified when HTTPS is enabled
* [ ] For `KeyMaterialProviderIsTestClass`:
...
* [ ] For `KeyMaterialProviderIsSelfSigned`:
...
* [ ] Configure MockWebServer with HTTPS using the appropriate SSL socket factory
* [ ] Implement logic to determine certificate source priority (test class method vs. self-signed)
* [ ] Add proper logging for HTTPS configuration steps
* [ ] Handle cleanup of resources in `afterEach` method
...
2. Implement two distinct approaches for key material provision:
   ...
   a. **KeyMaterialProviderIsTestClass**:
      ...
      * Method name specified via `keyMaterialProviderMethod` parameter (default: "provideKeyMaterial")
      * Method must return a `KeyMaterialHolder` or compatible type
      * Extension uses reflection to invoke the method on the test instance
      * Gives tests full control over certificate generation/loading
      * Example method signature: `public KeyMaterialHolder provideKeyMaterial()`
      * Example implementation:
        ...
        class CustomCertificateTest implements MockWebServerHolder {
            ...
            public KeyMaterialHolder provideKeyMaterial() {
                // Custom implementation to provide certificates
   ...
   b. **KeyMaterialProviderIsSelfSigned**:
